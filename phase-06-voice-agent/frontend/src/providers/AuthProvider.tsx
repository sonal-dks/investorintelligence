...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const run = async (session: Session | null) => {
      ...
      } catch { if (!cancelled) setProfile(fallbackProfileFromSession(session)); }
      finally { if (!cancelled) setLoading(false); }
    };

    const { data } = supabase.auth.onAuthStateChange(async (_event, session) => {
      if (!session) { reset(); navRef.current("/login", { replace: true }); return; }
      await run(session);
    });
    handoffSessionFromUrl()
  ...
  const signOut = async () => { await supabase.auth.signOut(); reset(); navRef.current("/login", { replace: true }); };
